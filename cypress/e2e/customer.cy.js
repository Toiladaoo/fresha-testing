...
describe('search services', () => {
  ...
  it('use current location',() =>{
    cy.visit(website);
    cy.get('.display-default-flex.gJ1v3P > [data-qa="location-search-input"]').click();
    cy.wait(4000)
    cy.get('.otggd7 > [data-qa="location-search"]').type('828 sư vạn hạnh phường 13 quận 10 thành phố hồ chí minh')
    cy.get('._8TeLm7').click();
    cy.wait(500);
    cy.get('[data-qa="search-button-mobile-variant-home"]').click();
    //check 
    cy.get('#button-search-labels-container').should('exist').contains('828');
  })
  it('Search by date and time',() => {
    ...
    cy.wait(500)
    //check 
    cy.get('#button-search-labels-container').should('exist').contains('Tomorrow')
  })
})
...
describe('add salon to favorite',() => {
  ...
  it('add na flower to favorite list ', () => {
    ...
    cy.get('.Grid-sc-14lvbxm-0').find('p').eq(0).should('include.text',"Na'Flower");
...
describe('booking',() => {
  ...
  it('booking one service',() => {
    ...
    cy.get('[data-qa="cart-mobile"] > .hB0YAs > .fWGp_B > .font-default-body-s-semibold').contains("Na'Flower");
    cy.get(':nth-child(1) > ._6lhlpX > ._002hAX > ._nTFgX > ._2rJOeX > .font-default-body-m-medium').should('have.text','Tuesday 12 December');
    cy.get('.gap-default-250x > :nth-child(3) > ._6lhlpX > ._002hAX > ._nTFgX > ._2rJOeX > ._-wK3ss').should('have.text', 'dao');
  ...
  it.only('booking many service',() => {
   ...
   cy.get('h1').contains('Review and confirm')
   cy.get('[data-qa="cart-mobile"] > .hB0YAs > .fWGp_B > .font-default-body-s-semibold').contains("Na'Flower");
   cy.get(':nth-child(1) > ._6lhlpX > ._002hAX > ._nTFgX > ._2rJOeX > .font-default-body-m-medium').should('have.text','Wednesday 13 December');
   cy.get('.gap-default-250x > :nth-child(3) > ._6lhlpX > ._002hAX > ._nTFgX > ._2rJOeX > ._-wK3ss').should('have.text', 'dao');
   cy.get('.K9pZ96 > .direction-default-horizontal').should('be.visible');
   cy.get('[data-qa="continue-button-mobile"]').click();
